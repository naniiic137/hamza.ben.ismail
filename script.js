...
class PortfolioApp {
    ...
    createAllSounds() {
        return {
            ...
            // UFO Invasion sounds - Enhanced
            ufoEngine: () => this.createUFOSound('engine'),
            ufoBeam: () => this.createUFOSound('beam'),
            ufoPass: () => this.createUFOSound('pass'),
            ufoScan: () => this.createUFOSound('scan'),
            ufoHover: () => this.createUFOSound('hover'),
            ufoFormation: () => this.createUFOSound('formation'),
    ...
    // Override createTone to respect mute state and sound enable state
    ...
    spawnRandomUFO() {
        ...
        // Enhanced sound sequence for random UFO
        this.sounds.ufoEngine(); // Initial engine sound
        setTimeout(() => {
            this.sounds.ufoHover(); // Hover sound
        }, 500);
        setTimeout(() => {
            this.sounds.ufoScan(); // Scanning sound
        }, 2000);
        setTimeout(() => {
            this.sounds.ufoBeam(); // Beam sound
        }, 4000);
        setTimeout(() => {
            this.sounds.ufoPass(); // Pass sound as it leaves
        }, 8000);
    ...
    setupEnhancedTypingEffect() {
        ...
        const typeNextCharacter = () => {
            ...
            // Add the character span
            typingText.appendChild(charSpan);
            
            // Add subtle typing effect without flickering
            charSpan.style.opacity = '0';
            charSpan.style.transform = 'scale(0.8)';
            
            // Smooth fade in for new character
            requestAnimationFrame(() => {
                charSpan.style.transition = 'opacity 0.1s ease, transform 0.1s ease';
            ...
            try {
                ...
                const response = await emailjs.send(
                    ...
                    'YOUR_TEMPLATE_ID', // Replace with your EmailJS template ID
                    {
                        ...
                        from_email: email,
                        message: message,
                        to_name: 'Hamza Ben Ismail', // Replace with your name
                        reply_to: email
                    }
                );
    ...
    // Create achievement card effect
    createAchievementCardEffect(item, index) {
        // Add floating particles around the card
        const particles = this.createParticles(item, 8);
        ...
        // Add achievement glow effect
        item.style.setProperty('--achievement-glow', '1');
        setTimeout(() => {
            item.style.setProperty('--achievement-glow', '0');
        }, 1000);
    }
    ...
    // Setup distant UFOs
    setupDistantUFOs() {
        const container = document.querySelector('.distant-ufos-container');
        if (!container) return;

        const createDistantUFO = () => {
            const ufo = document.createElement('div');
            ufo.className = 'distant-ufo';
            ...
            switch(edge) {
                case 0: // top
                    ...
                    moveY = Math.random() * window.innerHeight + 50;
                    break;
                case 1: // right
                    startX = window.innerWidth + 10;
                    startY = Math.random() * window.innerHeight;
                    moveX = -(Math.random() * window.innerWidth + 100);
            ...
            ufo.style.left = startX + 'px';
            ufo.style.top = startY + 'px';
            ufo.style.setProperty('--move-x', moveX + 'px');
            ufo.style.setProperty('--move-y', moveY + 'px');
