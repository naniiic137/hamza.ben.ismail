...
export const projects: Project[] = [
  ...
  },
  {
    ...
    features: [
      ...
      'Security: login throttling (429 + Retry-After), timing-safe login, bcrypt byte limits, CSP headers, optimistic locking',
      'Flyway + PostgreSQL (tested with Testcontainers), Docker Compose, CI with an end-to-end Docker smoke test',
    ],
    facts: [{ label: 'TESTS', value: '98' }, { label: 'COVERAGE', value: '86%' }, { label: 'STACK', value: 'SPRING + REACT' }],
    title: 'ApplyTrack',
    category: 'web',
    featured: true,
    summary:
  ...
  },
  {
    ...
    features: [
      ...
      'Reconnect within 60 s: a seat token restores your fogged board and move log; after that the player who stayed wins by abandonment',
      'Full board reveal at game end, a move log paired White/Black, sounds with a mute toggle, and a QR code to join',
      'Chaos mode with 6 fairy pieces (Amazon, Chancellor, Archbishop, Nightrider, Camel, Wizard), boards up to 10×10, threefold and 50-move draws',
      '11 automated tests (node:test + socket.io-client), including checks that the fog never leaks on reconnect',
    ],
    ...
    featured: true,
    summary:
      'Hidden-information chess for LAN play: enemy pieces are invisible — you only know a square is occupied. Secret setups, private guess-pins, fair check alerts, 60-second reconnection and a Chaos mode with fairy pieces (Amazon, Chancellor, Nightrider) on boards up to 10×10.',
    tech: ['Node.js', 'Socket.io', 'Express', 'Chess.js', 'node:test'],
    links: [gh('fog-chess')],
  },
  {
    id: 'chesscipher',
  ...
  },
  {
    ...
    features: [
      ...
      'Keys stay in the browser; strict CSP; 196 automated tests plus a golden-set evaluation (100% extraction precision); CI + GitHub Pages deploy',
    ],
    facts: [{ label: 'TESTS', value: '196' }, { label: 'PROVIDERS', value: '4' }, { label: 'COST', value: 'FREE' }],
    title: 'JobFit AI',
    category: 'web',
    featured: true,
    summary:
  ...
  },
  {
    ...
    features: [
      ...
      'A headless runner that plays scripted button presses: 320 test scripts (3,600 checks), and every screenshot and GIF in the README',
    ...
    facts: [{ label: 'GAMES', value: '12 / 40' }, { label: 'TEST CHECKS', value: '3,600' }, { label: 'LINES OF C', value: '44k' }],
    title: 'UFO 40',
    category: 'desktop',
    featured: true,
    summary:
  ...
  },
  {
    ...
    features: [
      ...
      "GitHub export: the theme carries onto github.com as SVG header and section images (fonts drawn as outlines, light/dark versions), themed badges and alerts",
      "Open any repo's README from a short link (…/readme-glow/github.com/owner/repo), a badge, a bookmarklet or the repo's Website field",
    ...
    facts: [{ label: 'TESTS', value: '1,379' }, { label: 'THEMES', value: '15' }, { label: 'LAYOUTS', value: '5' }],
    title: 'ReadmeGlow',
    category: 'web',
    featured: true,
    summary:
      "Turns any README.md into a designed web page (15 themes, 5 layouts), with two-way editing, Beautify and a summary, and carries the theme onto GitHub itself.",
  ...
  },
  {
    ...
    features: [
      ...
      '74 web tests (Vitest) + 28 firmware tests (unittest), CI and GitHub Pages deploy — testing on a real Pico still pending',
    ],
  ...
  },
  {
    id: 'kalak',
    overview:
      'A real-time multiplayer party game for phones, inspired by Kalak / Fibbage. Everyone writes a convincing fake answer to a trivia question, then tries to spot the real one among the bluffs — points for finding the truth and for fooling friends. Playable fully in Arabic or English: the room\'s language switches the questions and every player\'s interface, right-to-left included.',
    features: [
      'Rooms with 5-character codes, up to 12 players, host settings validated and clamped on the server',
      'Reload-proof seats: a per-browser token holds your seat for 60 s, and a reload resumes the same seat, score, answer and vote',
      'Nobody gets stuck: a round moves on as soon as every connected player has answered, and the host role passes on if the host leaves',
      'The real answer is shuffled with the bluffs server-side — clients never learn it early, and you can\'t vote for your own lie',
      'Identical bluffs (ignoring case, accents and Arabic letter variants) merge into one option that credits every author',
      '325 hand-written questions in 13 categories; full Arabic / English interface with RTL switching; tied scores share medals',
      '23 automated tests: unit tests plus end-to-end games played through socket.io-client',
    ],
    facts: [{ label: 'QUESTIONS', value: '325' }, { label: 'LANGUAGES', value: 'AR · EN' }, { label: 'TESTS', value: '23' }],
    title: 'Kalak (كلك)',
    category: 'web',
    summary:
      'Real-time multiplayer trivia-bluffing party game for phones (Arabic/English): write fake answers, spot the real one. Reload-proof seats, merged duplicate bluffs and 23 automated tests. Node.js + Socket.io.',
    tech: ['Node.js', 'Socket.io', 'Express', 'JavaScript', 'node:test'],
    links: [gh('Kalak')],
  },
  {
    id: 'checkers',
  ...
  },
  {
    id: 'microsaving',
    overview:
      "Give it a savings goal (for example 2,000 TND) and daily minimum/maximum amounts; it splits the goal into small random daily targets and generates trackers you can actually use — on PC, on your phone, or on paper.",
    features: [
      "One CLI with --format pc | phone | print | all",
      "Installable phone PWA (when hosted over https) with progress saved locally",
      "Printable A4 sheet",
      "Same seed → same plan, plus a parallel seed-recovery tool",
      "Input validation and 13 automated tests",
    ],
    facts: [{ label: "OUTPUTS", value: "PC · PHONE · PRINT" }, { label: "TESTS", value: "13" }],
    title: "MicroSaving",
    category: 'python',
    summary:
      "Turns a savings goal into small randomized daily amounts, then generates a PC tracker, an installable phone app (PWA) and a printable sheet.",
    tech: ["Python", "HTML", "CSS", "JavaScript", "PWA"],
  ...
  },
  {
    id: 'inventory',
    overview:
      "A desktop app for a small shop to record sales in a local SQLite database, search them, and export daily or full reports to CSV.",
    features: [
      "Add, update and delete sales with validation",
      "Real-time search by customer name",
      "Export today’s sales or everything to CSV",
      "Automatic running totals (TND)",
      "Database logic separated and unit-tested (8 tests)",
    ],
    facts: [{ label: "DATABASE", value: "SQLITE" }, { label: "TESTS", value: "8" }],
    title: "Simple Sales Manager",
    category: 'desktop',
    summary:
  ...
  },
  {
    id: 'valenlink',
    overview:
      "Generate a personal link to ask someone to be your Valentine; their answer is emailed straight to you. No backend — email is sent through EmailJS.",
    features: [
      "Creator mode generates a personal, validated link",
      "Multi-stage question where every “No” changes the text",
      "Runaway “No” button on desktop and touch screens",
      "Real error handling — success only when the email is confirmed sent",
      "Optional fixed-recipient mode so the page can only email one address",
    ],
    facts: [{ label: "BACKEND", value: "NONE" }, { label: "DELIVERY", value: "EMAILJS" }],
    title: "ValenLink",
    category: 'web',
    summary:
      "Playful Valentine’s link generator: send a personal link, get the Yes/No answer by email (EmailJS) — with validated links and a “No” button that runs away.",
  ...
  },
  {
    id: 'dice',
    overview:
      "Six dice roll automatically; the goal is to get all six showing the same number in as few rolls as possible. Scores go to a global leaderboard stored in SQLite.",
    features: [
      "Global top-10 leaderboard (Flask + SQLite)",
      "Player names rendered safely — no script injection",
      "Only the player who set a score can rename it (secret edit token, stored hashed)",
      "Server-side validation of names and scores",
      "15 automated tests with Flask’s test client",
    ],
    facts: [{ label: "BACKEND", value: "FLASK" }, { label: "DATABASE", value: "SQLITE" }, { label: "TESTS", value: "15" }],
    title: "Dice Game",
    category: 'python',
    summary:
      "Six dice roll until they all match — fewest rolls wins. Flask + SQLite web game with a global leaderboard, protected renames and a test suite.",
    tech: ["Python", "Flask", "SQLite", "JavaScript"],
    links: [{ label: 'Live', href: 'https://nanic137.pythonanywhere.com' }, gh('Dice_Game')],
  },
  {
    id: 'game-of-math',
    overview:
      "A classic Java web application: create an account, sign in, and answer arithmetic problems while the app keeps your score and streak. Built with plain Servlets and JSP, no framework.",
    features: [
      "Sign-up, login and logout with salted PBKDF2 password hashes",
      "Answers and scores kept in the server session — not in the page",
      "Problems always have whole-number answers; no division by zero",
      "Input validated and all output escaped",
      "Runs on Tomcat 7–9",
    ],
    facts: [{ label: "LANGUAGE", value: "JAVA" }, { label: "STACK", value: "SERVLETS · JSP" }],
    title: "Game Of Math",
    category: 'web',
    summary:
      "Java Servlet/JSP arithmetic quiz with sign-up and login (salted password hashes), session-based scoring and streaks.",
  ...
  },
  {
    id: 'love-calc',
    overview:
      "A playful Android app written in Delphi (FireMonkey): type two names and get a “love percentage”, computed from the characters of both names. Purely for fun.",
    features: [
      "Android app built with Delphi FireMonkey",
      "Name-based score from a character-code checksum",
    ...
    category: 'desktop',
    summary:
      "Just-for-fun Android app built with Delphi FireMonkey that turns two names into a “love percentage” using a character-code checksum.",
    tech: ["Delphi", "FireMonkey", "Android"],
